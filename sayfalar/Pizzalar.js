...
function Pizzalar(props) {
  ...
    };
    const totalFiyat = siparisListesi.reduce((acc, curr) => acc + curr.fiyat, 0);
    ...
    <button type="button" class="btn btn-light" style={{position:"right"}} onClick={() => GarsonCagir()}>Garson Çağır</button>
      ...
      <ul style={{listStyleType:"none"}}>
        {siparisListesi.map((siparis, index) => (
          <li key={index}>
            <center><img src={images[index]} className="card-img-top" style={{width: "20rem", height:"15rem"}} /><h5 style={{textAlign:"center",margin:"20px" }}>{siparis.baslik} - {siparis.fiyat}{"₺"}</h5>
